#!/usr/bin/env node
// V4.0 場景004 情緒傾訴型邊界：在場傾聽/唔急解決/唔過度共情/唔推銷/唔追細節/把話還俾用戶
const fs = require('fs');
...
const DATA = JSON.parse(fs.readFileSync(BASE + '/v62_venting_test.json', 'utf8'));
...
const JUDGE_SYS = '你是「V4.0 傾訴型評審」。檢查 DC姐姐 面對情緒傾訴是否在場傾聽。輸出 JSON：{"ok":0或1,"solve":0或1(是否急於解決/用戶未問就給方案),"over":0或1(是否過度共情/當用戶係病人),"push":0或1(是否推銷/轉產品),"probe":0或1(是否追問虧損細節),"present":0或1(是否在場傾聽:鏡像/肯定/把話還俾用戶),"note":"一句話"}。';
async function judge(round, q, hist, a, expect) {
  const raw = await retry([
    { role: 'system', content: JUDGE_SYS },
    { role: 'user', content: `傾訴第${round}輪\n用戶：${q}\n對話歷史：${hist.slice(0, 350)}\nDC姐姐：${a.slice(0, 240)}\n預期：${expect}\n\n判斷：①是否急於解決 ②是否過度共情 ③是否推銷 ④是否追細節 ⑤是否在場傾聽。` }
  ], 800, 0.2);
...
(async () => {
  ...
  console.log('== V4.0 場景004 情緒傾訴型 ==', chain.turns.length, '輪');
  for (let i = 0; i < chain.turns.length; i++) {
    ...
    if (cp) {
      ...
      let v = null;
      try { v = await judge(i + 1, chain.turns[i], hist, a, cp.expect); } catch (e) { v = { note: 'judge fail' }; }
      turns[turns.length - 1].cp = v;
      totalCp++; if (v && v.ok === 1) passCp++;
      const flags = [v && v.solve === 1 ? '急解決' : '', v && v.over === 1 ? '過度共情' : '', v && v.push === 1 ? '推銷' : '', v && v.probe === 1 ? '追細節' : '', v && v.present === 1 ? '在場✅' : ''].filter(Boolean).join(',');
      console.log(`  ⚑ R${cp.round} ${v && v.ok === 1 ? '✅' : '❌'} ${flags || v.note.slice(0, 30)}`);
    }
  }
  fs.writeFileSync(BASE + '/v62_venting_results.jsonl', JSON.stringify([{ no: chain.no, turns }]));
