...
const SITE = JSON.parse(fs.readFileSync('/Users/macbookair/Downloads/电子书ipa/网站/site_full_rebuild_simplified.js', 'utf8').replace('const SITE_FULL = ', '').replace(/;\s*$/, ''))
// 股路不歸/股道用繁體版（用戶要求暫時不變）
const KB_ORIG = JSON.parse(fs.readFileSync('/Users/macbookair/Downloads/电子书ipa/网站/dc-sister/kb-data.json', 'utf8'))
const TRAD_BOOKS = ['股路不歸·DC問答', '股道·DC問答']
for (const c of SITE) {
  if (TRAD_BOOKS.includes(c.name)) {
    const oc = KB_ORIG.categories.find(x => x.name === c.name)
    if (oc) { c.qa = oc.qa; c.icon = oc.icon || c.icon; c.description = oc.description || c.description }
  }
}
