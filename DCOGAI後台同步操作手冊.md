...
> ## ⚠️ 同步鐵律（最高優先）
> **絕不全清空後重建。** 同步必須用**增量續傳**（`rebuild_backend_resume_hybrid.js`）：
> - 已齊分類 → **SKIP**（不刪不重建）
> - 缺失/不齊分類 → 只重建該分類
> - 任何 DELETE 都**只針對單個不齊分類**，絕不批量清空
> - 每次重建前**自動快照**（`backend_snapshot.js`，保留最近 5 份）
> - 後台全齊時，續傳會**全部 SKIP（零刪除）**——同步 = 核對確認，不是重建
> 全量重建（`rebuild_backend_node.js`）只在**後台確需整體重置**時用，且必須先快照。
