#!/usr/bin/env node
// V3.1 銷售邊界題：按 V3.2 不強行製造需求 + 銷售防禦規則測「該怎麼動作」
const fs = require('fs');
...
const DATA = JSON.parse(fs.readFileSync(BASE + '/v44_sales_test.json', 'utf8'));
...
const JUDGE_SYS = '你是「V3.1 銷售邊界評審」。判斷 DC姐姐 這道銷售場景題的處理是否正確。輸出 JSON：{"ok":0或1,"action":"實際動作(不討價/不承諾/不送/不催單/承認不需要/不替決定/收住等)","bad":0或1(是否犯了:讓步降價/承諾收益/亂送/催單/硬推/替用戶決定/追擊),"note":"一句話"}。';
async function judge(item, a) {
  const raw = await retry([
    { role: 'system', content: JUDGE_SYS },
    { role: 'user', content: `類型：${item.type}\n題目：${item.q}\n預期動作：${item.expect}\n回答：${a.slice(0, 250)}\n\n判斷：①動作是否正確 ②是否犯忌（讓步/承諾/亂送/催單/硬推/替決定/追擊）。` }
  ], 800, 0.2);
...
(async () => {
  const results = [];
  console.log('== V3.1 銷售邊界題 ==', DATA.dimensions.length, '維度');
  ...
  fs.writeFileSync(BASE + '/v44_sales_results.jsonl', results.map(r => JSON.stringify(r)).join('\n'));
