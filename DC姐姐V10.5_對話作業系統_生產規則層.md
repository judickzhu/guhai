...
## 十六、[COG] 標記實測（2026-09-13）

**實測**：3 輪連續對話，LLM **全部未輸出 [COG|...] 標記**（「無」×3）。

**結論**：
1. `[COG]` 隱形標記機制**實際未生效**——LLM 不生成標記（prompt 寫「可選/附不上不勉強」），dc-sister.js 的解析邏輯拿不到
2. 但 **100/100 盲測仍通過**——說明 DC 靠 **LLM 對話歷史自然記憶** 維持連續認知（不依賴顯式標記）
3. 真正缺的仍是**代碼級 USER_STATE**（V10.5 完整字段）——若要可測/可存/跨會話，需重構

**啟示**：prompt 內化的「狀態驅動」實際是「LLM 隱式狀態」——效果有（盲測通過），但不可觀測/不可調試/跨會話弱。代碼級 USER_STATE 是後續升級方向（需開發）。
